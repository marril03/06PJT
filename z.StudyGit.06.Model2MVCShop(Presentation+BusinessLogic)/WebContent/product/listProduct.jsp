...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<tr class="ct_list_pop">
		...
		<%if(searchs.equals(name)){ %>
			<a href="/getProduct.do?prodNo=<%=productVO.getProdNo()%>&menu=<%=name%>"><%=productVO.getProdName()%></a>
		<%}else{ %>
			<a href="/updateProductView.do?prodNo=<%=productVO.getProdNo()%>&menu=<%=name%>"><%=productVO.getProdName()%></a>
		<%} %>
		</td>
		<td></td>
			...
			<c:if test="${param.menu eq 'manage'}">
				   <c:if test="${empty product.proTranCode}">
					...
					<c:if test="${product.proTranCode eq '0'}">
						구매완료  <a href="/updateTranCodeByProd.do?prodNo=${product.prodNo}&proTranCode=1">배송하기</a>
					</c:if>		
